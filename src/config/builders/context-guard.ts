...
export function buildContextGuardConfig(globalConfig: GlobalYaml): Required<NonNullable<GlobalYaml["context_guard"]>> {
  ...
  return {
    ...
    warn_ratio: source.warn_ratio ?? 0.6,
    danger_ratio: source.danger_ratio ?? 0.8,
    block_ratio: source.block_ratio ?? 0.95,
    absolute_block_tokens: source.absolute_block_tokens ?? 250000,
