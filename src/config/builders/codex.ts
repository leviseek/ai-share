import type { CodexCliConfig, CodexCliProvider, CodexMcpServer, McpYaml, McpServerSource } from "../../types.ts";
...
export const CODEX_CONFIG_GENERATED_HEADER =
  "# Generated by ai-share. Do not edit directly; change config/*.yaml instead.";
export const CODEX_INSTRUCTIONS_GENERATED_HEADER = "# Codex Instructions\n\nThis file is generated by ai-share.";

export function buildCodexCliConfig(config: ConfigSet, providerId: string, instructionsFile: string): CodexCliConfig {
  const modelId = config.global.model;
  const modelSource = config.models[modelId];
  const providerSource = config.providers.providers[providerId];
  if (!modelSource) throw new Error(`模型未定义：${modelId}`);
  if (!providerSource) throw new Error(`提供商未定义：${providerId}`);
  const windowsConfig = nonEmptyRecord(config.global.codex_windows);
  const shellEnvironmentPolicy = nonEmptyRecord(config.global.codex_shell_environment_policy);

  return {
    model: modelSource.model_name,
    model_provider: providerId,
    ...(modelSource.reasoning_effort ? { model_reasoning_effort: modelSource.reasoning_effort } : {}),
    model_instructions_file: instructionsFile,
    ...(config.global.codex_allow_login_shell === undefined
      ? {}
      : { allow_login_shell: config.global.codex_allow_login_shell }),
    ...(windowsConfig ? { windows: windowsConfig } : {}),
    ...(shellEnvironmentPolicy ? { shell_environment_policy: shellEnvironmentPolicy } : {}),
    model_providers: { [providerId]: buildCodexProvider(providerId, providerSource) },
    ...nonEmptyMcpServers(buildCodexMcpServers(config.mcp)),
  };
}
...
function buildCodexProvider(
  providerId: string,
  provider: ConfigSet["providers"]["providers"][string],
): CodexCliProvider {
  return {
...
function buildCodexMcpServers(mcpConfig: McpYaml): Record<string, CodexMcpServer> {
  return Object.fromEntries(
    Object.entries(mcpConfig.servers).map(([serverId, server]) => [serverId, buildCodexMcpServer(serverId, server)]),
  );
}
