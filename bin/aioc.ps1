...
function Test-SessionTokenOverflow {
  ...
  $tokens = 0
  $tmp = Join-Path ([IO.Path]::GetTempPath()) "aioc_guard_${PID}.mjs"
  try {
...
'@ -Encoding UTF8
  ...
  Write-Host (U @(32, 32, 24314, 35758, 26032, 24320, 32, 97, 105, 111, 99, 32, 25110, 20808, 23545, 26087, 20250, 35805, 25191, 34892, 31616, 35201, 20043, 31867, 30340, 21387, 32553, 21518, 20877, 24674, 22797, 12290)) -ForegroundColor Yellow
...
$null = Test-SessionTokenOverflow $OpenCodeArgs.ToArray() $ActiveConfig
