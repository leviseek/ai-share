...
"蒸馏"指将有价值的对话内容提取为结构化记忆，写入 ai-share 仓库内的 `memory/` 目录，使其在后续会话中自动注入到 AI 上下文。核心规则：**AI 提案，人工确认**。
...
| ai-share generator | 消费 `memory/` YAML 文件，编译为自然语言指令注入 session |
...
### 提案 3：删除 — stable/user.yaml → obsolete_section
