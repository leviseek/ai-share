...
`ai-share` centralizes OpenCode and oh-my-openagent configuration for multiple machines/projects. Bun + strict TypeScript generate user-level config from YAML sources; launcher scripts install `aiomo`, `aioc`, monitor, and context-guard tooling.
...
- `.worktrees/install-doctor/` is local worktree content; do not treat it as primary source.
