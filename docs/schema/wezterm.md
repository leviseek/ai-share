...
`ai:config` 仅支持 Windows 和 macOS。stdin 和 stdout 均为 TTY 时默认启动六步交互向导；非 TTY 或使用 `--non-interactive` 时使用仓库值。`--dry-run` 永不写入文件，但在 TTY 中仍会先运行六步交互向导，随后输出配置摘要和计划；`--force` 只显式接管未受管的目标普通文件。目标路径、所有权和安全边界见 [`README.md`](../../README.md) 的 WezTerm 配置说明。
