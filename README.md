> # ⚠️ 本项目已废弃（DEPRECATED）
>
> **废弃日期：2026-09-13。本仓库不再维护、不再接收改动。**
>
> `ai-share` 曾是面向 OpenCode CLI 的用户级配置生成器，其职责已被一套基于其他 AI 编码工具链的环境取代，长期记忆能力也已迁至独立知识库。
>
> 本仓库仅保留历史快照。下文文档描述的是项目停止维护前的状态，不再保证与任何现存工具版本兼容。请勿在此基础上继续开发，也不要提交 issue 或 PR。
