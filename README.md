...
`memory/` 目录存放用户级记忆文件（结构化 Markdown），在 OpenCode 启动时作为系统指令自动注入。它将你的编码风格、工作流偏好、工具链和 Agent 编排模式等个人记忆持久化到仓库，跨项目、跨设备使用。

```text
memory/
├── user/                    # 用户身份与偏好
├── architecture/            # 架构与设计哲学
├── stack/                   # 核心技术栈
├── stable/                  # 稳定记忆（来自独立 ai-memory 仓库迁移）
├── profiles/                # profile 特定记忆
├── policies/                # 记忆策略
├── inferred/                # 推断记忆（运行时生成）
├── distilled/               # 蒸馏记忆（长期摘要）
├── runtime/                 # 运行时记忆（gitignored）
...
ai-share 定义了可移植的三角色模型协议 (tri-role/v1)，将 agent/category 与具体模型 ID 解耦：

| 角色        | 用途                 |
| ----------- | -------------------- |
| `primary`   | 主编码/执行 agent    |
| `reasoning` | 深度推理、架构分析   |
| `fast`      | 轻量搜索、compaction |

agents 和 categories 不直接引用模型 ID（如 `gpt-5.5`），而是引用这三个抽象角色名。运行时通过当前 profile 将角色名解析为具体模型。`docs/protocol/tri-role.md` 包含完整协议规范（Schema、模型解析规则、fallback 链、跨工具适配器合约），TypeScript 类型和验证器见 `src/protocol/tri-role.ts`。

### 多工具适配器

基于 tri-role 协议，可将 profile 导出到其他 AI 工具供直接使用：

| 适配器         | 输出格式                          | 文件                                            |
| -------------- | --------------------------------- | ----------------------------------------------- |
| Cursor         | `.cursorrules`                    | `src/protocol/adapters/cursor-rules.ts`         |
| GitHub Copilot | `.github/copilot-instructions.md` | `src/protocol/adapters/copilot-instructions.ts` |
| Claude Code    | `CLAUDE.md`                       | `src/protocol/adapters/claude-code.ts`          |

每个适配器读取当前 profile 的角色模型映射，生成对应工具的原生配置文件。

### Profile 导入/导出

支持将 profile 导出为独立的 tri-role JSON 文件，供外部工具或备份使用：
...
导入时自动执行协议版本检查 (`version` 字段)、角色完整性验证 (`primary`/`reasoning`/`fast` 均不可为空) 和模型注册表引用完整性检查。

### 成本仪表板

基于 OMO agent monitor 采集的状态数据，生成终端表格和 HTML 报告：

```sh
# 终端表格
bun run src/cli/costs.ts

# HTML 报告（含图表）
bun run src/cli/costs.ts --html --output report.html

# 使用测试 fixture 验证
bun run src/cli/costs.ts --state-file src/__tests__/fixtures/omo-agent-monitor-state.json
```

成本仪表板展示每个模型的 token 消耗、按 profile 汇总的成本和预估月度支出，数据来源为 `~/.config/opencode/omo-agent-monitor-state.json`。

### LLM 增强摘要

Session 摘要工具支持两种模式：无模型参与的确定性提取和 LLM 增强分析：

```sh
# 确定性摘要（不调用模型）
bun run src/cli/summarize.ts ses_xxx

# LLM 增强摘要（5s 超时，失败自动回退确定性模式）
bun run src/cli/summarize.ts ses_xxx --llm
```

LLM 模式下优先使用 reasoning 角色模型做深度分析；超时或调用失败时自动回退到确定性摘要。

### Memory 关键词检索

`src/memory/retrieval.ts` 提供 TF-IDF 关键词检索，零外部依赖，直接在本地 memory 文件中搜索：

```ts
import { searchMemory } from "./src/memory/retrieval.ts";
const results = searchMemory("代理配置");
// [{ path: "memory/stack/opencode.md", score: 0.85, snippet: "..." }]
```

检索范围覆盖 `memory/` 目录下的所有结构化 Markdown 文件，结果按 TF-IDF 相关度排序。

### 插件 Manifest 自注册

插件目录下放置 `ai-share-plugin.json` 即可被生成器自动发现，无需修改安装代码：

```json
{
  "name": "my-plugin",
  "version": "1.0.0",
  "permissions": ["fs:read", "network:localhost"],
  "aioc_compatible": true
}
```

`bun run ai:check` 会输出插件权限扫描结果，标注高风险权限 (`network:any`、`fs:write`、`bash:allow` 等)。未被 `config/global.yaml` 引用的插件不会自动安装，需显式纳入 `optional_plugins`。
